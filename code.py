...
def send_sms(message, from_number, recipient_list, fail_silently=False, auth_user=None, auth_password=<PASSWORD>, connection=None):
	"""
	...
	"""
	connection = connection or get_sms_connection(username=auth_user, password=<PASSWORD>password, fail_silently=fail_silently)
	mail = SMSMessage(message, from_number, recipient_list, connection=connection)

	return mail.send()


def send_mass_sms(datatuple, fail_silently=False, auth_user=None, auth_password=<PASSWORD>, connection=None):
	"""
	...
	connection = connection or get_sms_connection(username=auth_user, password=<PASSWORD>, fail_silently=fail_silently)
